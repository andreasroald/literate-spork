...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, collide_list, npc_list, display):
        ...
        self.image = self.player_list[0]
        self.image_original = self.image.copy()
        self.image_rect = self.image.get_rect()
        self.rect = self.image_rect.copy()
        self.rect.inflate_ip((-40,-40))
        self.rect.center = (display_width/2, display_height/2+100)
    ...
    def movement(self):
        ...
        # adjust speed if player is sneaking
        if self.sneaking:
            self.speed = 2
        ...
        # change player list if sneaking
        if self.sneaking:
            self.player_list = sneaking_list
        else:
            self.player_list = player_list
        ...
        if self.moving_up or self.moving_left or self.moving_down or self.moving_right:
            if not self.in_conversation and not self.killing_npc:
                ...
                if self.walk_counter >= self.update_delay:

                    self.walk_counter = 0
                    self.image_original = self.player_list[self.walk_number]
                    self.walk_number += 1
...
class NPC(pygame.sprite.Sprite):
    ...
    def __init__(self, position_lists, face, lines, collide_list, player, id):
        ...
        self.see_zone_width = 150
        self.see_zone_height = 350
        self.see_zone_offset = 0
        self.see_zone = pygame.Rect((0, 0, self.see_zone_width, self.see_zone_height))
    ...
    def update(self):
        ...
        # Hold gun if seeing player
        if self.seeing_player:
            self.image_original = enemy_gun

        # Make ncp look at player
        self.old_rect  = self.image_rect.copy()

        if self.seeing_player:

            self.image, self.image_rect = look_at(self.player_pos[0], self.rect.center[0], self.player_pos[1],
                                                  self.rect.center[1], self.image, self.image_original, self.old_rect)
        ...
        if self.direction == 0:
            self.see_zone.width = self.see_zone_width
            self.see_zone.height = self.see_zone_height
            self.see_zone.midbottom = self.rect.center
            self.see_zone.y += self.see_zone_offset
        ...
        if self.direction == 90:
            self.see_zone.width = self.see_zone_height
            self.see_zone.height = self.see_zone_width
            self.see_zone.midright = self.rect.center
            self.see_zone.x += self.see_zone_offset
