...
class Game:
    ...
    def update(self):
        ...
        if self.player.moving_up or self.player.moving_left or self.player.moving_down or self.player.moving_right:
            if not self.player.in_conversation and not self.player.killing_npc:
                if self.current_time - self.prev_footstep > self.footstep_delay:
                    self.prev_footstep = pygame.time.get_ticks()
                    self.player.sound_level = 40
                    if self.player.sneaking:
                        self.footstep_1.set_volume(0.2)
                        pygame.mixer.Sound.play(self.footstep_1)
                    else:
                        self.footstep_1.set_volume(1)
                        pygame.mixer.Sound.play(self.footstep_1)

        # resize the NPC vision zone if the player is sneaking
        if not self.player.sneaking:
            for resize_vision_zone in self.current_room.npc_list:
                resize_vision_zone.see_zone_width = 300
                resize_vision_zone.see_zone_height = 650
                resize_vision_zone.see_zone_offset = 100
        else:
            for resize_vision_zone in self.current_room.npc_list:
                resize_vision_zone.see_zone_width = 150
                resize_vision_zone.see_zone_height = 350
                resize_vision_zone.see_zone_offset = 0
    ...
    def draw(self):
        ...
        # show text if player is sneaking
        if self.player.sneaking:
            render_text_center("SNEAKING", red, self.game_display, y=-250, size="medium")
